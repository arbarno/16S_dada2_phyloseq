## This script provides the workflow to raw demultiplexed Illumina Reads using DADA2 with cutadapt primer removal
...
# The 341F - 806R 16S primer pair will generate an amplicon set with a bimodal length distribution.
# We will only keep sequence reads with a length of 400-430 bp.
seqtab.filtered <- seqtab[,nchar(colnames(seqtab)) %in% seq(400,430)]
...
hist(nchar(getSequences(seqtab.filtered)), main="Distribution of sequence lengths")
...
# Set seed for replicability (already happened for error rate estimation, repeat if pipeline has been terminated in between).
...
track <- cbind(out, sapply(dadaFs, getN), sapply(dadaRs, getN), sapply(mergers,getN), rowSums(seqtab.nochim),rowSums(seqtab.nochim.nosingle))
colnames(track) <- c("input", "filtered", "denoisedF", "denoisedR", "merged", "nonchim","no_single")
rownames(track) <- basename(filtFs)
track
write.table(track,"track_16S.txt",sep="\t",col.names = NA)
